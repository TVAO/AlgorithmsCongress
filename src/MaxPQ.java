...
public class MaxPQ<Key> implements Iterable<Key> {
    ...
    public static void main(String[] args) {
        ...
        State[] stateList = loader.loadStates();
        MaxPQ<State> PQ = new MaxPQ<>();
        
        for (State state: stateList) {
            PQ.insert(state);
            state.addSeat(); // Assign one seat to each state first time
        }
        
        double seats = loader.SEATS;
        int states = loader.STATES;
        String stateName;

        seats-=states; // Remaining seats when 50 is subtracted from house total
        
        State state;
        while(seats > 0) {
            state = PQ.max();
            state.addSeat();
            seats--; // One less seat left
            state.decreasePopulation((int)(state.getPopulation()/Math.sqrt(seats*(seats+1))));
            // Does the current max state heap have greatest population size 
            if (!PQ.isMaxHeap()) {
                // state = PQ.delMax();
                // PQ.insert(state);
                // PQ.max().compareTo(PQ.getKey(PQ.ma));
            }
        }
        
        Iterator<State> iterator = PQ.iterator();
        while (iterator.hasNext()) {
            state = iterator.next();
            seats = state.getSeats();
            stateName = state.getStateName();
            System.out.format("%s has total amount of seats %d %n", stateName, (int) seats);
        }
    }
